...
static TaskHandle_t xBluetooth_task_hanle;
#ifdef SUPPORT_HEARTBEAT
...
void spp_cmd_task(void * arg)  
{
    ...
    for(;;){
        ...
        if(xQueueReceive(cmd_cmd_queue, &cmd_id, portMAX_DELAY)) {
            ...
            // ESP_LOGI(GATTS_TABLE_TAG,"raw data: %s\n",cmd_id);
            if(cmd_id[0]==0xff){
                uint16_t temp =0;
                temp  = cmd_id[1]<<4;
                temp  |=cmd_id[2];
                ESP_LOGI(GATTS_TABLE_TAG,"raw data : %x,%x\n",(uint8_t)cmd_id[1],(uint8_t)cmd_id[2]);
                ESP_LOGI(GATTS_TABLE_TAG,"set v : %x\n",temp);
...
static void spp_task_init(void)  //  SPP 接受任务
{
    spp_uart_init();
    ...
    xTaskCreate(spp_cmd_task, "spp_cmd_task", 2048, NULL, 10, &xBluetooth_task_hanle);
}
...
void ble_start(void ){
    esp_err_t ret;
    ESP_LOGI(GATTS_TABLE_TAG, "%s start bluetooth\n", __func__);

    // 启用Bluedroid栈
    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(GATTS_TABLE_TAG, "启用Bluedroid失败: %s", esp_err_to_name(ret));
        esp_bt_controller_disable(); // 回滚操作
        return ;
    }
    ...
    esp_err_t ret;
     // 1. 禁用 Bluedroid
        if ((ret = esp_bluedroid_disable()) != ESP_OK) {
            ESP_LOGE(GATTS_TABLE_TAG, "禁用 Bluedroid 失败: %s", esp_err_to_name(ret));
            return;
        }
     
        esp_bt_controller_mem_release(ESP_BT_MODE_BTDM);
       
        vQueueDelete(cmd_cmd_queue);    //删除队列
        vTaskDelete(xBluetooth_task_hanle);  //删除蓝牙任务
}