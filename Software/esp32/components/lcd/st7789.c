...
// {
//     gpio_set_level(ST7789_DC_GPIO, 0); // 命令模式
//     spi_transaction_t t = {
//         .length = 8,
//         .tx_buffer = &cmd,
//     };
...
// {
//     gpio_set_level(ST7789_DC_GPIO, 1); // 数据模式
//     spi_transaction_t t = {
//         .length = len * 8,
//         .tx_buffer = data,
//     };
...
//  * * Soft reset by using command 
//  */
// void st7789_reset(void)
// {
//     st7789_send_cmd(0x01);

//     vTaskDelay(10 / portTICK_PERIOD_MS); // 等待150ms
//     st7789_send_cmd(0x01);
//     vTaskDelay(10 / portTICK_PERIOD_MS); // 等待150ms
// }
...
// {
//     // 初始化GPIO
//     gpio_set_direction(ST7789_DC_GPIO, GPIO_MODE_OUTPUT);
//     gpio_set_direction(ST7789_BACKLIGHT_GPIO, GPIO_MODE_OUTPUT);
//     gpio_set_direction(ST7789_CS_GPIO, GPIO_MODE_OUTPUT);

//     // SPI总线配置
//     spi_bus_config_t buscfg = {
//         .mosi_io_num = ST7789_MOSI_GPIO,
//         .miso_io_num = -1,
//         .sclk_io_num = ST7789_SCLK_GPIO,
//         .quadwp_io_num = -1,
//         .quadhd_io_num = -1,
...
//         .queue_size = 3
//     };
//     ESP_ERROR_CHECK(spi_bus_initialize(ST7789_SPI_HOST, &buscfg, SPI_DMA_DISABLED));
//     ESP_ERROR_CHECK(spi_bus_add_device(ST7789_SPI_HOST, &devcfg, &st7789_spi));

//     st7789_reset();

//     // 初始化命令序列（部分常用命令）
//     st7789_send_cmd(0x36); uint8_t data1 = 0x60; st7789_send_data(&data1, 1); // MADCTL
//     st7789_send_cmd(0x3A); uint8_t data2 = 0x05; st7789_send_data(&data2, 1); // COLMOD: 16bit
//     st7789_send_cmd(0x11); vTaskDelay(120 / portTICK_PERIOD_MS); // Sleep Out
...
{
     st7789_set_window(0, 0, 239, 319); // 设置全屏窗口（240x320）
    uint8_t data[2] = {color >> 8, color & 0xFF};
...
void st7789_test_screen(void)
{
    

   while(1){
        st7789_clear(0x0000); // 清屏为黑色

        // 点亮屏幕中心的一个像素（红色）
        st7789_draw_pixel(120, 160, 0xF800); // 240x320屏幕中心，RGB565红色

        // 填充一条横线（绿色）
        for (int x = 0; x < 240; x++) {
            st7789_draw_pixel(x, 60, 0x07E0); // 绿色
        }

        // 填充一条竖线（蓝色）
        for (int y = 0; y < 320; y++) {
            st7789_draw_pixel(60, y, 0x001F); // 蓝色
        }
